...
the .lock-xxx files at the root of the repository.")
...
`lock-stable'."
  (if (or (string-equal "master" (spacemacs//git-get-current-branch))
      ...
      (configuration-layer/load-file configuration-layer-lock-file-stable)
    (configuration-layer/load-file configuration-layer-lock-file-latest)))
...
(defun configuration-layer//stable-elpa-installed-directory ()
...
(defun configuration-layer//stable-elpa-installed-p ()
  "Returns non-nil if stable ELPA repository is installed."
  (file-exists-p
...
(essentially for testing purpose).
"
  (let ((can-continue t))
    (when (and configuration-layer-stable-elpa-name
               (not (configuration-layer//stable-elpa-installed-p)))
      (setq can-continue (configuration-layer//stable-elpa-download-tarball))
      (when (and can-continue
